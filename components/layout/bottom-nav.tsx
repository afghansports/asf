...
const ITEMS: { href: string; label: string; icon: typeof Home; match: (p: string) => boolean }[] = [
  { href: "/",            label: "Home",    icon: Home,          match: (p) => p === "/" },
  { href: "/feed",        label: "Wall",    icon: Sparkles,      match: (p) => p.startsWith("/feed") },
  { href: "/reels",       label: "Reels",   icon: Video,         match: (p) => p.startsWith("/reels") },
  { href: "/discussions", label: "Talk",    icon: MessageSquare, match: (p) => p.startsWith("/discussions") },
